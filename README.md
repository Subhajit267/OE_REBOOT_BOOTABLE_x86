...
*This document covers the entire bare-metal kernel generation built on top of v8.42.28. Read the prior READMEs for the pre-kernel story; read this one for everything that happened between v8.42.28 and v10.22.37.*
...
**OE_REBOOT v10.22.37** is the point where the long-stated "future roadmap" line that every prior README carried — *"Adding kernel, Building bootloader, HAL (arm, arm64, x86, x64)"* — stopped being a roadmap item and became real, working, booting code.

Everything documented in the prior READMEs (the full hosted OE_REBOOT app: login, user management, installer, registry editor, settings, notepad, calculator, systeminfo, PAL for Windows/Linux) is **unchanged in behavior** in v10.22.37. What's new is a **third PAL backend** (`pal_kernel.c`) that satisfies the exact same `pal.h` contract as `pal_windows.c`/`pal_linux.c`, sitting on top of a from-scratch:
...
The result: the same login screen, the same `Command->` shell, the same apps — now running with **zero host operating system underneath**, verified booting in QEMU, Virtual Box and VMWare, and installable to a real disk image that boots on its own via a custom bootloader.
...
Highlights since the last documented snapshot (v8.42.28):
...
- 🐛 **Real, previously-latent bugs found and fixed** only once real hardware paths were exercised: an ANSI→VGA color-palette mapping bug, a VGA line-wrap double-advance bug, a keyboard raw/cooked queue cross-contamination bug (notepad "garbage first line"), an out-of-range ACPI table dereference causing a shutdown page-fault on non-QEMU hypervisors, and a missing explicit VGA cursor-shape program.
- 📄 **Full project-standard file headers** added across the entire `kernel/`, `hal/`, `pal_kernel.c`, `installer/`, `boot/`, `bootloader/` tree (~43 files).
...
### 2. Standalone Bootloader Chain (no GRUB, no CD-ROM)
...
### 5. Disk Installer — Install vs Try Mode
...
flowchart TD
    ...
    I -- Yes, non-destructive --> J[pal_init + scoped built-in shell]
    I -- No --> K[Report: no filesystem found]
    J -- "install command" --> D
    H --> L[Reboot with no CD-ROM attached]
    L --> M[Standalone bootloader chain]
    M --> N["installer_prompt(): pal_is_bare_metal() true -> skip Y/N -> run_installation()"]
    N --> O[User creation wizard]
    O --> P["Real login() screen on next boot"]
```

### 6. `kernel_main` Real Sequencing
...
The following phases document the kernel development process. The core shell and PAL architecture were established since 2022 and are entirely independent of the kernel work. An earlier in-progress kernel/HAL/bootloader attempt was lost to an SSD failure with no backup, so the kernel subsystem was rebuilt from scratch — which is why its file dates run newer than the rest of the codebase. `kernel/`, `hal/`, and `boot/` were empty directories at the start of Phase 1, and `pal/src/pal_kernel.c` was a 0-byte stub.
...
First real implementation of the PAL contract for bare metal: `heap.c` (kmalloc/krealloc/kfree, first-fit free-list over a static 1MB arena), cooked+raw keyboard input queues, `pal_file_open_read` backed by real FAT reads. Deferred (not stubbed silently): `pal_math.h`, `pal_oe_info.h`, `pal_dir_file_cmds.h`, file writes — all closed out in Phase 7. Verified live via injected keystrokes + `screendump`.

### Phase 7 — FULL PORT ✅ (the biggest single phase)
The **entire hosted OE_REBOOT app now boots and runs natively**. In order:
...
2. **`pal_kernel.c` completed** — full `pal_math.h` (hand-rolled `atof`/`ftoa`, a from-scratch Taylor-series `pal_pow`), full `pal_oe_info.h` (CPUID brand string), full `pal_dir_file_cmds.h`.
...
5. **Two toolchain fixes**: linking `libgcc.a` for real 64-bit division (`systeminfo.c`), and a `FPU_OBJS` Makefile list stripping `-mgeneral-regs-only` for the handful of files doing genuine double-precision math.
6. **Two real, previously-latent `vga.c` bugs** exposed: an ANSI-to-VGA color palette mapping bug (blue rendered as red) and a missing deferred/lazy line-wrap (double-advancing the row, corrupting every other row's background).
7. **`kernel_main` now runs the real flow**, matching the hosted `main.c` line for line.

Verified end-to-end via QEMU: bootscreen → installer wizard → user creation → same app running bare metal.

### Phase 8 — FAT32 + MBR + Subdirectories + Bootloader + Installer ✅
1. **FAT32** — `fat.c` rewritten with a generalized `struct fat_dir`, FAT32 4-byte entries, `fat_format()` FAT32 path (fatgen103 table, FSInfo, root at cluster 2). Both FAT16 and FAT32 brand OEM/volume label as **"OEFS"** — standard, `mtools`-readable.
2. **Subdirectories** — path-aware API (`fat_resolve_path`, `fat_mkdir_in`, `fat_rmdir_in`); real `.`/`..` dirents; `pal_kernel.c` gained real `mdr`/`rdr`/`cdr`/`pwd`/`ldr`/`cpf`/`mvf`/`rmf`/`rdf`.
...
4. **Standalone bootloader** — `bootloader/src/mbr.asm` + `stage2.asm`: real-mode → E820 → A20 → flat GDT → protected mode → chunked kernel load → synthesized `multiboot_info` — same handoff contract as GRUB, zero `kernel_main` changes needed. Verified booting with no `-cdrom` at all.
5. **Isolated disk installer** — `installer/src/installer_main.c`, embeds `mbr.bin`/`stage2.bin`/`kernel.flat.bin` at build time, writes them + a fresh partition + OEFS/FAT32 filesystem to a target disk.
6. **Full acceptance chain verified**: installer.iso installs to a scratch disk → CD-ROM detached → reboot on disk alone → standalone bootloader → OOBE wizard → user created → reboot → real `login()` screen.

**Deferred by design**: non-destructive install alongside existing partitions — intentionally left for a future update.

### Phase 9 — Gap-closure, Install/Try Menu, Follow-up Fixes ✅
...
- `installer_main.c` gained `[1] Install` / `[2] Try (Guest Mode)` menu.
- Try Mode non-destructively mounts an existing filesystem and drops into a scoped built-in shell without touching the disk destructively unless `install` is explicitly run.
- **Notepad "garbage first line" bug fixed** — root cause: the keyboard IRQ handler pushed every keystroke into both the cooked and raw input queues unconditionally. Fixed with `keyboard_flush_raw()`/`keyboard_flush_cooked()` called on mode transitions.
- **`shutdown` page-fault crash fixed** — `acpi.c` dereferenced ACPI table pointers assuming they live below the 256MB identity-map cap. Added `acpi_addr_safe()` gating every dereference.
- **VGA cursor shape bug fixed** — `vga_set_cursor_visible()` now explicitly programs cursor shape (CRTC 0x0A/0x0B) via `vga_set_cursor_shape(13, 14)`.
- **Full project-standard file headers** added across the entire `kernel/`, `hal/`, `pal_kernel.c`, `installer/`, `boot/`, `bootloader/` tree (~43 files).
...
| Kernel-level hardening pass | – | – | – | – | ⚠️ open (see Roadmap) |

---

## 🧱 System Modules

All modules from v8.42.28 are unchanged. New in v10.22.37:
...
> Root also carries build artifacts: `os.iso`, `disk.img`, `bootdisk.img`, `kernel.bin`, `kernel.flat.bin`, `mbr.bin`, `stage2.bin`, `installer.bin`, `installer.iso` — all regenerated by `make`.
...
| `make vmware` | `disk.vmdk` | Use with the checked-in `OE_Reboot.vmx` — IDE controller |
...
See the prior READMEs — plain `gcc`/module-include build, or the `OE REBOOT V1` Visual Studio project. Nothing about this changed in v10.22.37.
...
```
Attach the produced `disk.vmdk`/`disk.vdi` and `os.iso` to an **IDE** controller (not SATA/SCSI/NVMe) — see Known Issues for two fixes still pending real-hypervisor confirmation.

---

## 🐞 Known Issues / Open Items

Carried forward from prior releases (unaffected by the kernel work):
- `login.c` plaintext-password debug prints are commented out, not deleted.
- `pal_dir_file_cmds_linux.c` has no shell-metacharacter guard on `pal_cpdr`.
- `user_id_change.c` accepts an empty new username, no length/charset validation.
- The hardcoded `"registryeditor"`/`"guess_it"` login backdoor is **intentionally kept**.

New, kernel-specific:
- Heap (`kernel/heap.c`) has no bounds checking or guard pages.
- FAT write has no journaling/atomicity — a mid-write power loss can corrupt the volume.
- No input-length audit pass has been done on the freshly-ported hosted code.

**Pending real-hypervisor retest** (fixed and QEMU-verified, but not yet retested on VMware/VirtualBox):
- Cursor rendering fix (`vga_set_cursor_shape`) for VMware.
...
- FAT32 formatting path not separately re-tested through the installer specifically.
- E820 memory-map handling untested against a fragmented map.
- Pure UEFI-only (no CSM/BIOS compatibility) hardware is out of scope by design.
...
### Immediate next step
Hardening pass: heap bounds/guard pages, FAT write atomicity, input-length audit over the freshly-ported code, and closing remaining security items from prior releases.

### Explicitly deferred
Non-destructive install alongside existing partitions — intentionally left for a future update.

### Long-term vision
- A ring-3/scheduler phase, if/when multitasking is actually wanted.
- HAL support for additional architectures (ARM/ARM64/x64) — the `hal.h` boundary established in Phase 4 makes this possible without touching `kernel/` or the hosted app.
- Turning `pal_kernel.c`'s plain function calls into real trap-based syscalls if/when a ring-3 boundary is introduced.

---

## 📋 Project History & Prior Versions

The core shell was originally designed way back in school using C and C++. An earlier in-progress kernel/HAL/bootloader attempt was lost to an SSD failure with no backup, so the kernel subsystem was rebuilt from scratch — which is why its file dates run newer than the rest of the codebase.

For the kernel development, AI was used for boilerplate and guidance since OS/kernel development was new territory — the same way Stack Overflow was leaned on during the original school-era development. Not every line of the kernel subsystem is hand-written, but real bugs like the ANSI-to-VGA color mismatch, the Notepad stray-character keyboard IRQ bug, and the ACPI shutdown fault were debugged and fixed directly, verified live in QEMU, mtools, VMware, and VirtualBox.

**Prior versions and oldest sources:**
- [2020–2022 website](https://sites.google.com/view/operatingenvironment/) — the original site hosting the oldest versions
- [Original Windows C/C++ version](https://github.com/Subhajit267/Operating-Environment-for-windows-C-and-Cpp-based)
- [Original Linux version (ran on onlinegdb)](https://github.com/Subhajit267/Original-Operating-Environment-2020-Aug-Jul-cpp-variant-for-linux-)
- [OE-REBOOT — the C rewrite with PAL architecture](https://github.com/Subhajit267/OE-REBOOT)
...
|---|---|---|
| **v10.22.37** | 2026-09-02 | Bare-metal kernel generation: own kernel/HAL/bootloader, FAT12/16/32 + MBR, disk installer with Install/Try menu, `pal_kernel.c` fully implemented. Same hosted app, zero behavior changes, now also boots with no host OS. |
| **v8.42.28** | 2026-08-26 | Full sources for Notepad, Calculator, System Info. Salted SHA-256 password hashing. Filesystem commands. Terminal size standardised to 80×25. |
| **v8.13.07** | 2026 | Notepad, Calculator, System Information Tool introduced; PAL-backed filesystem command layer; centralized branding system. |
| **v6.48.21** | 2026-02-24 | Settings panel, registry editor admin mode, guest mode, colour wizard, reset & restore, dedicated help system. |
| **v6.12.56** | 2026-02-21 | Initial C port: PAL, user management, installer, table-driven prompt, basic registry editor. |
...
Contributions, bug reports, and suggestions are warmly welcomed! Please follow the existing code style — including the project's file-header convention — and document your changes.
